...
public class EyeTracking_Nik : MonoBehaviour {
    ...
    private static readonly string[] Columns = { "CaptureTime", "HeadsetPos", "HeadsetRotation", "CombinedGazeForward", "FocusDistance", "FocusStability", "CalcXEccentricity", "CalcYEccentricity" };
    ...
    public void Update() {
        if (Globals.GetKeyDown(calibrationKey))
            CalibrateGaze();

        if (Globals.GetKeyDown(loggingToggleKey)) {
            if (!logging)
                StartLogging();
            else
                StopLogging();
    ...
    public void StartLogging() {
        ...
        string logPath = Globals.dataPath + fileName + ".csv";
    ...
    void LogGazeData(VarjoEyeTracking.GazeData data) {
        ...
        logData[0] = Globals.varjoTime(data.captureTime).ToString();
