...
S = {'o', 'd', 's', 'h', '^', '*', '*', '^', 'h', 's', 'd', 'o', 'o', 'd', 's', 'h', '^', '*', '*', '^'};
refD = str2num(refD{1});
axes{2} = axNames{1};

%% ignores column of data for separated scaling or eccentricity plots
% Set = 2 for Eccentricity analysis
% Set = 3 for Scaling analysis
ignoreCol=2;
...
%{
    ...
    %{ COMMENTED OUT BECAUSE SCALING VALUES +/- 1 from kink value (need at
    %least +/- 2 values)
